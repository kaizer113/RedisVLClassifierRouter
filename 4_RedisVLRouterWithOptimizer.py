#!/usr/bin/env python3
"""
RedisVL SemanticRouter with Threshold Optimization.
- Uses RouterThresholdOptimizer to find optimal distance thresholds for each route
- Evaluates performance with optimized thresholds
- Compares with ChatGPT fallback for unknown classifications
"""
...
def prepare_optimizer_test_data(articles):
    """
    Prepare test data in the format required by RouterThresholdOptimizer.
    Format: [{"query": "text", "query_match": "expected_route_name"}, ...]
    """
    test_data = []
    for article in articles:
        test_data.append({
            "query": article['text'],
            "query_match": article['actual_category']
        })
    return test_data
...
def main():
    """Main function to run classification with threshold optimization."""
    print("=" * 80)
    print("BBC News Classification - RedisVL Router with Threshold Optimization")
    print("=" * 80)
    print()
    ...
    # Step 2: Create routes with initial threshold
    print("\nStep 2: Creating routes with initial threshold...")
    initial_threshold = 0.5
    routes = create_routes(references_by_category, distance_threshold=initial_threshold)
    print(f"  Initial distance_threshold: {initial_threshold}")

    # Step 3: Initialize router
    print("\nStep 3: Initializing SemanticRouter...")
    ...
    # Step 4: Read test articles for optimization
    print("\nStep 4: Reading test articles for threshold optimization...")
    # Use a subset for optimization (faster)
    optimization_articles = read_test_articles('BBC News Train.csv', num_articles=100)
    print(f"  Loaded {len(optimization_articles)} articles for optimization")

    # Step 5: Prepare data for optimizer
    print("\nStep 5: Preparing test data for optimizer...")
    optimizer_test_data = prepare_optimizer_test_data(optimization_articles)

    # Step 6: Run threshold optimization
    print("\nStep 6: Running threshold optimization...")
    print("  This may take a few minutes...")
    print(f"  Initial thresholds: {router.route_thresholds}")

    optimizer = RouterThresholdOptimizer(
        router=router,
        test_dict=optimizer_test_data,
        eval_metric="precision"  # Can also use "f1" "precision" or "recall"
    )

    # Run optimization with custom parameters
    optimizer.optimize(
        max_iterations=20,  # Number of random search iterations
        search_step=0.20    # Search range around current threshold
    )

    print(f"  Optimized thresholds: {router.route_thresholds}")

    # Step 7: Read full test set for evaluation
    print("\nStep 7: Reading full test set for evaluation...")
    test_articles = read_test_articles('BBC News Train.csv', num_articles=200)
    print(f"  Loaded {len(test_articles)} test articles")
    
    # Step 8: Classify with optimized router
    print("\nStep 8: Classifying articles with optimized router...")
    print("-" * 80)
    ...
    for route_name, threshold in router.route_thresholds.items():
    ...
    if router_times:
        ...
        print(f"  P95: {p95_router:.3f}s")
        print(f"  P99: {p99_router:.3f}s")
